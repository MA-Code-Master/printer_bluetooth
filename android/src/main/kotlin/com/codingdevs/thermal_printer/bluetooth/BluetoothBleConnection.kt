...
class BluetoothBleConnection(
    ...
    // MALY-POS FORK: returns whether the write was actually issued, to match
    // IBluetoothConnection.write. Previously Unit, which let a no-op look like
    // a successful print. See BluetoothConnection.write for the full story.
    override fun write(out: ByteArray?): Boolean {
        val characteristic = mCharacteristic ?: return false
        val gatt = bluetoothGatt ?: return false
        characteristic.writeType = BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE
        characteristic.value = out
        val issued = gatt.writeCharacteristic(characteristic)
        if (issued) {
    ...
    @Suppress("unused")
    private fun writeLegacy(out: ByteArray?) {
