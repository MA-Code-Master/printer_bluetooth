...
class BluetoothPrinterConnector implements PrinterConnector<BluetoothPrinterInput> {
  ...
  /// MALY-POS FORK: reports whether the disconnect actually happened.
  ///
  /// Upstream returned a hard-coded `false` even on success, so a caller could
  /// not tell a closed socket from one that refused to close — and a stale
  /// half-open socket makes every subsequent connect() time out.
  @override
  Future<bool> disconnect({int? delayMs}) async {
    try {
      if (Platform.isAndroid) {
        return await flutterPrinterChannel.invokeMethod('disconnect') ?? true;
      } else if (Platform.isIOS) {
